...
main (int argc, char *argv[])
{
  ...
  uint8_t stachannelNumber = wifiPhyPtr->GetChannelNumber();
  std::cout << "before setting it, the channel number=" << int(stachannelNumber) << std::endl;
  ...
  Ptr<NetDevice> apDev = apDevices.Get(0);
  Ptr<WifiNetDevice> apnetdev = DynamicCast<WifiNetDevice> (apDev);
  Ptr<WifiPhy> apPhyPtr = apnetdev->GetPhy();
  uint8_t apchannelNumber = apPhyPtr->GetChannelNumber();
  uint16_t apchannelWidth = apPhyPtr->GetChannelWidth();
  std::cout << "The AP's channel number is " << int(apchannelNumber) << " and channel width is " << apchannelWidth << std::endl;
