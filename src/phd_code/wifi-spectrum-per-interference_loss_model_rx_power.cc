  ...
  for (uint16_t i = startIndex; i <= stopIndex; i++)
    {
      ...
      friis->SetAttribute ("Frequency", DoubleValue (5.180e9));
      ...
      *wgPsd = waveformPower / 20e6;  // PSD spread across 20 MHz
      ...
      Ptr<MobilityModel> position_a = wifiApNode.Get(0)->GetObject<MobilityModel> ();
      Ptr<MobilityModel> position_b = wifiStaNode.Get(0)->GetObject<MobilityModel> ();
